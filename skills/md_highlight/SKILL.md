...
| 文本类型 | 核心标注要素 | 颜色对应 |
|----------|--------------|----------|
| **叙事类**（小说、故事、散文、日记） | 人名（专色，便于快速扫读）、关键事件转折点、核心动机/愿望、关键时间/地点 | 人名→浅蓝；转折/主旨→浅黄；时间/地点→浅绿；动机/愿望→浅橙 |
| **科普类**（自然、生物、天文、地理） | 主体对象、关键时间节点、核心行为/习性/特征、核心目的/意义 | 主体/概念→浅蓝；时间→浅绿；重要提示→浅橙 |
| **技术类**（教程、农业、工业、工具） | 关键参数、核心工具/设备、安全要点、核心结构/材料 | 参数→浅绿；工具/结构→浅蓝；安全→浅橙 |
| **说明类**（规则、流程、通知、指南） | 核心条件、关键规则、必须/禁止执行的动作、核心要求 | 规则/条件→浅蓝；必须/禁止→浅橙 |
| **其他**（新闻、议论文、随笔） | 核心观点、关键论据、核心事件、关键数据 | 沿用通用映射 |

## 标记格式（唯一允许格式）

1. **文件头部**：在正文首行前插入 `<style>` 块，定义所有高亮 class（仅一次，若已有则勿重复）：
```html
<style>
...
2. **标注用法**：按类型使用对应 class，禁止内联 style：
- 浅蓝（术语/概念/人名）→ `<span class="hl-concept">`
- 浅黄（结论/主旨）→ `<span class="hl-conclusion">`
- 浅绿（数据/时间）→ `<span class="hl-data">`
- 浅橙（重要提示）→ `<span class="hl-warning">`
...
8. **禁止标注引述套话**：如「研究表明」「我们认为」「据某报告」等，仅表示来源/态度，非实质结论，不标
...
```
（术语=hl-concept，重要提示=hl-warning；本句仅2处标注，每处≤4字）
